...
const Settings = () => {
  ...
  const showSidebar = !isMobile || activeTab === null;
  const showContent = !isMobile || activeTab !== null;
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col pb-20">
      ...
      <main className="relative z-10 flex-1 flex overflow-hidden" style={{ height: "calc(100vh - 72px)" }}>
        {/* Sidebar */}
        {showSidebar && (
          <div className={`${isMobile ? "w-full" : "w-[240px] min-w-[200px]"} border-r border-border/30 bg-card/40 backdrop-blur-md p-3 flex flex-col gap-1`}>
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {tab.label}
                  {isMobile && <ChevronRight className="w-4 h-4 ml-auto text-muted-foreground" />}
                </button>
              );
            })}
          </div>
        ...
        {showContent && activeTab && (
          <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
            {isMobile && (
              <button onClick={handleBack} className="flex items-center gap-2 text-sm text-muted-foreground mb-4 hover:text-foreground transition-colors">
